...
function storeAuth(username: string) {
  updateStoredAuthUsername(username);
}
...
export function Auth({ onLogin }: AuthProps) {
  ...
  /**
   * 使用通行密钥登录。通行密钥是独立的登录入口，不要求先填写用户名；
   * 当浏览器支持发现式凭据时，系统会直接列出当前设备可用的通行密钥。
   */
  async function handlePasskeyLogin() {
    if (loading) return;
    setLoading(true);
    try {
      const res = await authenticateWithWebAuthn(username, rememberMe);
      ...
      const meRes = await getUserInfo();
      storeAuth(meRes.username || res.username || username.trim());
      toast.success(t("messages.loginSuccess"));
      onLogin(
        meRes.username || res.username || username.trim(),
        meRes.userId || res.userId || undefined,
        !!(meRes.is_admin || res.is_admin),
      );
    } catch (err: unknown) {
      const error = err as {
        message?: string;
      ...
      const webAuthnErrorKey = getWebAuthnErrorTranslationKey(err);
      // 不把浏览器的 NotAllowedError 等内部英文堆栈显示给用户；服务端
      // 明确返回的原因仍保留，便于排查 RP ID、域名或凭据不匹配问题。
      toast.error(
        webAuthnErrorKey
          ? t(webAuthnErrorKey)
          : error?.response?.data?.error ||
              (error?.message &&
              !/NotAllowedError|AbortError/i.test(error.message)
                ? error.message
  ...
  const TAB_ITEMS: { id: AuthView; label: string; show: boolean }[] = [
    {
      ...
      // 通行密钥登录不依赖密码开关；即使管理员关闭密码登录，仍要保留
      // 登录页让用户使用已注册的通行密钥。
      show: !firstUser,
    },
    {
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-background overflow-hidden">
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Right panel */}
        <div className="flex min-w-0 flex-1 items-center justify-center overflow-y-auto px-4 py-6 sm:p-6 relative">
          <div className="flex min-w-0 w-full max-w-sm flex-col gap-6">
            {/* TOTP view */}
            {view === "totp" && (
              <div className="flex flex-col gap-5">
                ...
                <form onSubmit={handleTOTP} className="flex flex-col gap-4">
                  ...
                  <div className="relative my-1 flex items-center justify-center">
                    <Separator />
                    <span className="absolute bg-background px-2 text-[10px] uppercase tracking-normal text-muted-foreground">
                ...
                {view === "login" && (
                  <form onSubmit={handleLogin} className="flex flex-col gap-4">
                    ...
                    <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                      ...
                      {passwordResetAllowed && (
                        <button
                          ...
                          className="justify-self-start text-xs text-muted-foreground hover:text-accent-brand transition-colors sm:justify-self-end"
                        >
                          {t("auth.forgotPassword")}
                        </button>
                      )}
                    ...
                    <Button
                      ...
                      disabled={loading || !passwordLoginAllowed}
                    >
                      {loading ? (
                        t("common.loading")
                      ) : (
