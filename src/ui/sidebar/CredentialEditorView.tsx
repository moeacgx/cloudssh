...
export function CredentialEditorView({
  ...
  const [copyingSecret, setCopyingSecret] = useState<
    "password" | "key" | "keyPassword" | null
  >(null);

  const copyAdminSecret = async (field: "password" | "key" | "keyPassword") => {
    if (!adminTargetUserId || !credential) return;
    setCopyingSecret(field);
    try {
      const value = await adminCopyUserCredentialSecret(
        adminTargetUserId,
          ...
          <div className="flex flex-col gap-4 py-3">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between gap-2">
                ...
                {adminTargetUserId && credential && credForm.password && (
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    className="h-6 px-2 text-[10px]"
                    disabled={copyingSecret !== null}
                    onClick={() => void copyAdminSecret("password")}
                  >
                    ...
                    {adminTargetUserId &&
                      ...
                      credForm.value !== "existing_key" && (
                        <Button
                          ...
                          className="h-6 px-2 text-[10px]"
                          disabled={copyingSecret !== null}
                          onClick={() => void copyAdminSecret("key")}
                        >
                          <Copy className="size-3 mr-1" /> {t("common.copy")}
                        </Button>
                      )}
                  ...
                  {adminTargetUserId &&
                    ...
                    credForm.passphrase !== "existing_key_password" && (
                      <Button
                        ...
                        className="h-6 px-2 text-[10px]"
                        disabled={copyingSecret !== null}
                        onClick={() => void copyAdminSecret("keyPassword")}
                      >
                        <Copy className="size-3 mr-1" /> {t("common.copy")}
                      </Button>
                    )}
