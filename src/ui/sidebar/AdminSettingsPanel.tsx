...
export function AdminSettingsPanel({
  ...
  focusRequest?: { section: AdminSection; requestId: number } | null;
  onFocusHandled?: (requestId: number) => void;
} = {}) {
  const { t } = useTranslation();
  const [openSections, setOpenSections] = useState<Set<AdminSection>>(
    () => new Set([focusRequest?.section ?? "general"]),
  );

  useEffect(() => {
    if (!focusRequest) return;
    setOpenSections(new Set([focusRequest.section]));
  ...
  const [terminalRetentionValue, setTerminalRetentionValue] = useState("24");
  const [terminalRetentionUnit, setTerminalRetentionUnit] = useState<
    "minutes" | "hours" | "days"
  >("hours");
  ...
  async function loadGeneralSettings() {
    try {
      ...
      if (terminalSessionSettings.status === "fulfilled") {
        const timeoutMinutes = terminalSessionSettings.value.timeoutMinutes;
        if (timeoutMinutes % 1440 === 0) {
          setTerminalRetentionValue(String(timeoutMinutes / 1440));
          setTerminalRetentionUnit("days");
        } else if (timeoutMinutes % 60 === 0) {
          setTerminalRetentionValue(String(timeoutMinutes / 60));
          setTerminalRetentionUnit("hours");
        } else {
          setTerminalRetentionValue(String(timeoutMinutes));
          setTerminalRetentionUnit("minutes");
        }
      }
  ...
  async function handleSaveTerminalRetention() {
    const value = Number(terminalRetentionValue);
    const multiplier =
      terminalRetentionUnit === "days"
        ? 1440
        : terminalRetentionUnit === "hours"
          ? 60
          : 1;
    const timeoutMinutes = value * multiplier;
    if (
      ...
      toast.error(t("admin.sshDisconnectRetentionInvalid"));
      return;
    }
    try {
      await updateTerminalSessionSettings(timeoutMinutes);
      toast.success(t("admin.sshDisconnectRetentionSaved"));
    } catch {
      toast.error(t("admin.sshDisconnectRetentionSaveFailed"));
    }
  }
  ...
  return (
    <div className="flex flex-col gap-2 p-3 flex-1 min-h-0 overflow-y-auto">
      <AdminGeneralSettingsSection
        ...
        terminalRetentionValue={terminalRetentionValue}
        setTerminalRetentionValue={setTerminalRetentionValue}
        terminalRetentionUnit={terminalRetentionUnit}
        setTerminalRetentionUnit={setTerminalRetentionUnit}
        handleSaveTerminalRetention={handleSaveTerminalRetention}
