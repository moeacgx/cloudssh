...
const ADMIN_HOST_SECRET_FIELDS: AdminHostSecretField[] = [
  "password",
  "sudoPassword",
  "rdpPassword",
  "vncPassword",
...
export function HostEditor({
  ...
  const [form, setForm] = useState(() => {
    const initial = createHostEditorForm(host);
    return host || !initialFolder
      ? initial
      : { ...initial, folder: initialFolder };
  });
  const formWasEdited = useRef(false);

  const setField = <K extends keyof typeof form>(k: K, v: (typeof form)[K]) => {
    formWasEdited.current = true;
    setForm((p) => ({ ...p, [k]: v }));
  };
  ...
  const [showQuickCredentialDialog, setShowQuickCredentialDialog] =
    useState(false);
  const [adminSavedSecrets, setAdminSavedSecrets] = useState<
    Partial<Record<AdminHostSecretField, string>>
  >({});
  const [copyingHostSecret, setCopyingHostSecret] =
    useState<AdminHostSecretField | null>(null);
  ...
  useEffect(() => {
    ...
    getHostDefaults()
      .then((d) => {
        // 默认配置可能晚于用户输入返回，绝不能用它覆盖已经填写的密码或主机信息。
        if (cancelled) return;
        setForm((current) =>
          applyHostDefaultsToNewHostForm(
            current,
  ...
  useEffect(() => {
    if (!adminTargetUserId || !host?.id) return;

    let cancelled = false;
    setAdminSavedSecrets({});
    adminGetUserHostSecrets(
      adminTargetUserId,
      Number(host.id),
      ADMIN_HOST_SECRET_FIELDS,
    )
      .then((secrets) => {
        if (cancelled) return;
        setAdminSavedSecrets(secrets);
        setForm((previous) => ({
          ...previous,
          ...(secrets.password ? { password: secrets.password } : {}),
          ...(secrets.key ? { key: secrets.key, keySubTab: "paste" } : {}),
          ...(secrets.keyPassword ? { keyPassword: secrets.keyPassword } : {}),
          ...(secrets.sudoPassword
            ? { sudoPassword: secrets.sudoPassword }
            : {}),
          ...(secrets.rdpPassword ? { rdpPassword: secrets.rdpPassword } : {}),
          ...(secrets.vncPassword ? { vncPassword: secrets.vncPassword } : {}),
          ...(secrets.telnetPassword
            ? { telnetPassword: secrets.telnetPassword }
            : {}),
        }));
      })
      .catch(() => {
        if (!cancelled) toast.error(t("hosts.failedToLoadServer"));
      });

    return () => {
      cancelled = true;
    };
  }, [adminTargetUserId, host?.id, t]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const loadVncPassword = getHostPassword(Number(host.id), "vncPassword");
  ...
  const handleSave = async () => {
    if (isSshPasswordRequiredForSave(form, protocols, host?.authType)) {
      toast.error(t("hosts.passwordRequired"));
      return;
    }
    setSaving(true);
    try {
      ...
      if (adminTargetUserId) {
        saved = host
          ? await adminUpdateUserHost(
              adminTargetUserId,
              Number(host.id),
              Number(host.projectHostId),
              data,
            )
          : await adminCreateUserHost(adminTargetUserId, data);
      } else {
        if (host) {
          const projectMetadata = host.projectHostId
            ? buildProjectHostMetadataInput(data, activeProject.kind)
            : null;
          const preserveSharedMetadata =
            activeProject.kind === "team" && projectMetadata !== null;
          const globalData = preserveSharedMetadata
            ? preserveGlobalHostMetadata(data, host)
            : data;
          if (projectMetadata) {
            saved = await updateSSHHost(Number(host.id), globalData, {
              projectId: activeProject.id,
              projectHostId: Number(host.projectHostId),
              ...projectMetadata,
            });
            saved = {
              ...saved,
          ...
          saved = await createSSHHost(data, activeProject.id);
        }
      }
  ...
  const canQuickCreateCredential =
    ...
    (authMethod === "password"
      ? !!form.password || form.passwordSaved
  ...
  const copyAdminHostSecret = async (field: AdminHostSecretField) => {
    if (!adminTargetUserId || !host?.id) return;
    setCopyingHostSecret(field);
    try {
      const value = await adminCopyUserHostSecret(
        adminTargetUserId,
        Number(host.id),
        field,
      );
      ...
      toast[copied ? "success" : "error"](
        t(copied ? "common.copied" : "common.copyFailed"),
      );
    } catch {
      toast.error(t("common.copyFailed"));
    } finally {
      setCopyingHostSecret(null);
    }
  };

  const canCopySavedHostSecret = (
    ...
    currentValue === adminSavedSecrets[field];
  ...
  const handleQuickCreateCredential = async () => {
    ...
    try {
      const fetchField = (field: "password" | "key" | "keyPassword") =>
        adminTargetUserId
          ? adminCopyUserHostSecret(adminTargetUserId, Number(host?.id), field)
      ...
      if (authMethod === "password") {
        ...
        data.password =
          form.password ||
          (form.passwordSaved ? await fetchField("password") : null);
      } else {
        const key =
        ...
        <div className="flex flex-col gap-3">
          {activeTab === "general" && (
            <HostEditorGeneralTab
              ...
              hosts={hosts}
              host={host}
              projectId={adminTargetUserId ? undefined : activeProject.id}
                    ...
                    {authMethod === "password" && (
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {canCopySavedHostSecret(
                            ...
                            <Button
                              ...
                              disabled={copyingHostSecret !== null}
                              onClick={() =>
                                void copyAdminHostSecret("password")
                              }
                            >
                        ...
                        <PasswordInput
                          ...
                          placeholder={
                            form.passwordSaved && !form.password
                        ...
                        <div className="flex flex-col gap-1.5 col-span-2">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1">
                              {canCopySavedHostSecret("key", form.key) && (
                                <Button
                                  type="button"
                                  variant="ghost"
                                  size="sm"
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {canCopySavedHostSecret(
                              ...
                              <Button
                                ...
                                disabled={copyingHostSecret !== null}
                                onClick={() =>
                                  void copyAdminHostSecret("keyPassword")
                                }
                              >
                                <Copy className="size-3 mr-1" />
                                {t("common.copy")}
          ...
          {activeTab === "terminal" && (
            <>
              <SectionCard
                title={t("hosts.sessionPersistence")}
                icon={<Pin className="size-3.5" />}
              >
                <div className="flex flex-col gap-4 py-3">
                  <SettingRow
                    label={t("hosts.startPinnedWindow")}
                    description={t("hosts.startPinnedWindowDesc")}
                  >
                    <FakeSwitch
                      checked={form.startPinned}
                      onChange={(v) => {
                        setField("startPinned", v);
                        if (v) setField("autoTmux", false);
                      }}
                    />
                  </SettingRow>
                </div>
                ...
                <div className="flex flex-col gap-4 py-3">
                  ...
                  >
                    <FakeSwitch
                      ...
                      onChange={(v) => {
                        setField("autoTmux", v);
                        if (v) setField("startPinned", false);
                      }}
                    />
                  </SettingRow>
                  <SettingRow
