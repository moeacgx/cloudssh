...
export function AdminUserManagePanel({
  ...
  const [openingCredentialId, setOpeningCredentialId] = useState<string | null>(
    null,
  );
  ...
  usePageVisibleInterval(
    reloadHosts,
    3_000,
    dataUnlocked && hasPendingHostNetworkInfo(hosts),
    { runOnMount: false },
  );
  ...
  const openCredentialEditor = async (credential: Credential) => {
    ...
    try {
      const details = await adminGetUserCredentialDetails(
        user.id,
      ...
      setEditor({
        kind: "credential",
        credential: mapCredentialDetails(details),
      });
    } catch (error) {
      toast.error(apiErrorMessage(error, t("admin.credentialLoadFailed")));
    } finally {
  ...
  const manageTabs: { id: ManageTabId; label: string }[] = [
    ...
    { id: "hosts", label: t("admin.manageTabPersonalSpace") },
                ...
                >
                  ...
                  <div className="flex items-center gap-0.5 shrink-0">
                    <Button
                      ...
                      size="icon"
                      className="size-6 text-muted-foreground hover:text-foreground"
                      disabled={openingCredentialId === cred.id}
                      onClick={() => void openCredentialEditor(cred)}
                    >
                      <Pencil className="size-3" />
                    </Button>
                    <Button
