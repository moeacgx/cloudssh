...
export function createHostEditorForm(
  ...
  return {
    ...
    passwordSaved: !!host?.hasPassword,
    ...
    startPinned: host?.terminalConfig?.startPinned ?? false,
...
export function applyHostDefaultsToNewHostForm(
  current: HostEditorForm,
  defaults: HostDefaults,
  initialFolder: string | undefined,
  wasEdited: boolean,
): HostEditorForm {
  if (wasEdited) return current;
  const defaultForm = createHostEditorForm(null, defaults);
  return initialFolder
    ? { ...defaultForm, folder: initialFolder }
    : defaultForm;
}

export function isSshPasswordRequiredForSave(
  form: HostEditorForm,
  protocols: HostProtocols,
  originalAuthType?: HostAuthType,
): boolean {
  if (!protocols.enableSsh || form.authType !== "password" || !!form.password) {
    return false;
  }

  // 凭据认证解析出的密码也会表现为“已有密码”，但它并不是主机自身的
  // 直连密码。切换到密码认证时必须重新输入，避免移除凭据引用后留下
  // 一个没有任何可用密码的主机。
  return !form.passwordSaved || originalAuthType !== "password";
}

export function preserveGlobalHostMetadata(
  payload: SSHHostData,
  host: Host,
): SSHHostData {
  return {
  ...
  return {
    ...
    password:
      usesPassword || usesKey || usesCredential
        ? form.password || (form.passwordSaved ? undefined : null)
        : null,
    key: usesKey
    ...
    terminalConfig: protocols.enableSsh
      ? {
          ...
          autoTmux: form.startPinned ? false : form.autoTmux,
          startPinned: form.startPinned,
