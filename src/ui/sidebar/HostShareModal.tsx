...
function canManageProjectHost(role: WorkspaceRole) {
  return (
    role === "instance_admin" ||
    role === "team_admin" ||
    role === "project_admin"
...
export function HostShareModal({
  ...
  const [targetTab, setTargetTab] = useState<"user" | "role" | "project">(
    "user",
  );
  ...
  const targetProjects = useMemo(() => {
    const q = search.trim().toLowerCase();
    const hostId = host ? Number(host.id) : null;
    return projects.filter(
      (project) =>
        !isFolderShare &&
        project.id !== activeProject.id &&
        canManageProjectHost(project.role) &&
        (hostId === null || !project.hostIds.includes(hostId)) &&
        (!q || project.name.toLowerCase().includes(q)),
    );
  }, [activeProject.id, host, isFolderShare, projects, search]);

  const selectedCount =
    targetTab === "project"
      ? selectedProjectIds.size
      : selectedUserIds.size + selectedRoleIds.size;
      ...
      try {
        const results = await Promise.allSettled(
          [...selectedProjectIds].map((projectId) =>
            associateWorkspaceProjectHost(projectId, Number(host.id)),
          ),
        );
        ...
        if (failed.length === results.length) {
          throw new Error(t("hosts.sharing.projectShareFailed"));
        }
        setSelectedProjectIds(new Set());
        // 一个事件同时刷新项目清单、当前资产树及其它主机消费者，避免重复请求。
        window.dispatchEvent(new CustomEvent("termix:hosts-changed"));
        toast.success(
          t("hosts.sharing.projectShareSuccess", {
            count: results.length - failed.length,
          }),
        );
        if (failed.length > 0) {
          toast.error(
            t("hosts.sharing.projectSharePartial", { count: failed.length }),
          );
        }
      } catch (error) {
        toast.error(
          error instanceof Error
            ? error.message
            : t("hosts.sharing.projectShareFailed"),
        );
      } finally {
        setSubmitting(false);
      }
    ...
    <div className="absolute inset-0 z-20 flex flex-col bg-sidebar">
      ...
      <div className="flex flex-col gap-2 p-3 shrink-0 border-b border-border">
        ...
        <div className="flex gap-1.5">
          {(["user", "role", ...(host ? ["project"] : [])] as const).map(
            (tab) => (
              <button
                key={tab}
              ...
              >
                ...
                {tab === "project" && selectedProjectIds.size > 0 && (
                  <span>({selectedProjectIds.size})</span>
                )}
              </button>
            ),
          )}
        ...
        <div className="flex flex-col border border-border h-28 overflow-y-auto">
          ...
          {targetTab === "project" &&
            (targetProjects.length === 0 ? (
              <div className="px-3 py-4 text-xs text-muted-foreground/50 text-center">
                {t("hosts.sharing.noProjectMatches")}
              </div>
            ) : (
        ...
        {targetTab !== "project" ? (
          ...
          <div className="space-y-2">
            <p className="text-[11px] leading-snug text-muted-foreground">
              {t("hosts.sharing.projectShareDescription")}
            </p>
            <Button
              variant="outline"
              className="h-8 w-full border-accent-brand/40 text-accent-brand hover:bg-accent-brand/10 hover:text-accent-brand"
        ...
        {targetTab !== "project" && expiryPreset === "custom" && (
          <Input
            type="number"
            autoFocus
