...
export function HostsPanel({
  ...
  const projectHostIds = new Set(
    hostTree ? flattenHosts(hostTree).map((host) => Number(host.id)) : [],
  );
  const projectRawHosts = rawHosts.filter((host) =>
    projectHostIds.has(Number(host.id)),
  );
  const allTags = [
    ...new Set(projectRawHosts.flatMap((host) => host.tags ?? [])),
  ];
  ...
  async function handleExportHosts(share = false) {
    try {
      const result = await exportAllSSHHosts(
        share
          ? { share: true, hostIds: [...projectHostIds] }
        ...
        <div className="flex flex-col px-2 py-1.5 shrink-0 border-b border-border/60 gap-1.5">
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              try {
                ...
                if (result.associationFailed > 0) {
                  toast.error(
                    t("hosts.projectAssociationFailed", {
                      count: result.associationFailed,
                    }),
                  );
                }
              } catch (err: unknown) {
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="text-xs">
                <DropdownMenuItem
                  ...
                  disabled={projectFolderActions?.canManage === false}
                >
                ...
                <DropdownMenuItem
                  ...
                  disabled={
                    ...
                    !projectRawHosts.some((host) => host.enableProxmox)
                  }
                >
                ...
                <DropdownMenuItem
                  ...
                  disabled={projectRawHosts.length === 0}
                >
            ...
            <button
              ...
              className="flex items-center gap-1 h-7 px-2 text-[10px] font-medium text-accent-brand hover:bg-accent-brand/10 border border-accent-brand/30 rounded-sm shrink-0 transition-colors disabled:cursor-not-allowed disabled:opacity-40"
            >
          ...
          onProxmoxDiscover={
            projectFolderActions?.canManage === false
              ...
              : (host) => {
                  ...
                  setProxmoxDefaultCredentialId(
                    cfg?.defaultCredentialId ?? null,
                  );
