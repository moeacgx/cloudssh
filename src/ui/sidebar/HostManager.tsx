...
import { getWorkspaceProjectServers } from "@/api/workspace-api";
...
export function HostManager({
  ...
  const previousProjectIdRef = useRef(activeProject.id);
  const [editingHost, setEditingHost] = useState<Host | "new" | null>(null);
  const [newHostFolder, setNewHostFolder] = useState("");
  ...
  const reloadHosts = useCallback(() => {
    ...
    Promise.all([getSSHHosts(), getWorkspaceProjectServers(requestedProjectId)])
      .then(([raw, projectServers]) => {
      ...
      .catch(() => {});
  }, [activeProject.id, applyPendingEdit]);

  usePageVisibleInterval(reloadHosts, 3_000, hasPendingHostNetworkInfo(hosts), {
    runOnMount: false,
  });
  ...
  useEffect(() => {
    if (previousProjectIdRef.current === activeProject.id) return;
  ...
  useEffect(() => {
    if (!active) return;
    const handleAddHost = (event: Event) => {
      const detail = (event as CustomEvent<{ folder?: unknown }>).detail;
      setNewHostFolder(typeof detail?.folder === "string" ? detail.folder : "");
      setEditingHost("new");
      setEditingCredential(null);
      setEditingProtocols({
        enableSsh: true,
          ...
          {isHost ? (
            <HostEditor
              key={
                editingHost === "new"
                  ? "new-host"
                  : `${(editingHost as Host).id}:${(editingHost as Host).projectHostId ?? "global"}`
              }
              host={editingHost === "new" ? null : (editingHost as Host)}
              initialFolder={editingHost === "new" ? newHostFolder : undefined}
              activeTab={activeHostTab}
              onBack={() => {
                setEditingHost(null);
                setActiveHostTab("general");
