...
export function HostEditorGeneralTab({
  ...
  React.useEffect(() => {
    ...
    // 项目切换时立即移除旧候选项，避免请求完成前显示上一个项目的文件夹。
    setFolderMeta(new Map());

    const load = () => {
      const currentVersion = ++requestVersion;
      const request = adminTargetUserId
        ? getAdminUserPersonalProjectFolders(adminTargetUserId)
        : projectId
          ? getWorkspaceProjectFolders(projectId)
          : null;
      if (!request) {
  ...
  }, [adminTargetUserId, projectId]);

  const projectScoped = Boolean(projectId || adminTargetUserId);

  // 同时保留主机现有路径和项目空文件夹，并补齐多级路径中的父目录。
    ...
    <>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-3">
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
              {projectScoped ? t("hosts.projectFolder") : t("hosts.folder")}
            </label>
            <FolderPathPicker
              value={form.folder}
