...
import { getMfaStepUpMethods, type MfaStepUpMethod } from "@/api/mfa-api";
...
export function UserProfilePanel({
  ...
  const [passkeyStepUpOpen, setPasskeyStepUpOpen] = useState(false);
  const [passkeyStepUpMethods, setPasskeyStepUpMethods] = useState<
    MfaStepUpMethod[]
  >(["webauthn", "totp"]);
  const pendingPasskeyActionRef = useRef<{
    action: () => Promise<void>;
    fallback: string;
    translateWebAuthnError: boolean;
  } | null>(null);
  ...
  useEffect(() => {
    getUserInfo()
      .then((info) => {
        ...
        setUsernameDraft(info.username);
        if (info.username !== usernameRef.current) {
          onUsernameChangeRef.current?.(info.username);
        }
  ...
  async function handleUsernameUpdate() {
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{2,31}$/.test(nextUsername)) {
      toast.error(t("newUi.sidebar.userProfile.usernameFormatError"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("newUi.sidebar.userProfile.usernameUpdated"));
    } catch (error) {
      const status = (error as { status?: number })?.status;
      toast.error(
        status === 409
          ? t("newUi.sidebar.userProfile.usernameConflict")
          : status === 403
            ? t("newUi.sidebar.userProfile.usernameManagedBySso")
            : t("newUi.sidebar.userProfile.usernameUpdateFailed"),
      );
    } finally {
  ...
  async function runPasskeyAction(
    ...
    try {
      ...
      const methods = allowStepUp ? getMfaStepUpMethods(e) : null;
      if (methods) {
        pendingPasskeyActionRef.current = {
          action,
          fallback,
          translateWebAuthnError,
        };
        setPasskeyStepUpMethods(methods);
        setPasskeyStepUpOpen(true);
        return;
      }
      const webAuthnErrorKey = translateWebAuthnError
        ? getWebAuthnErrorTranslationKey(e)
        : null;
      toast.error(
        webAuthnErrorKey ? t(webAuthnErrorKey) : apiErrorMessage(e, fallback),
      );
    } finally {
  ...
  async function handleDeletePasskey(credentialId: string) {
    await runPasskeyAction(
      ...
      t("newUi.sidebar.userProfile.passkeyDeleteFailed"),
      false,
    );
  }

  async function finishPasskeyStepUp() {
    const pendingAction = pendingPasskeyActionRef.current;
    pendingPasskeyActionRef.current = null;
    setPasskeyStepUpOpen(false);
    if (!pendingAction) return;
    await runPasskeyAction(
      pendingAction.action,
      pendingAction.fallback,
      pendingAction.translateWebAuthnError,
      false,
    );
  }
    ...
    <div className="flex flex-col gap-2 p-3">
      ...
      >
        <div className="flex flex-col gap-0 pt-2">
          <div className="grid grid-cols-2 gap-x-4 gap-y-0">
            <div className="flex flex-col py-2">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <Input
                  value={usernameDraft}
                  disabled={(isOidc && !isDualAuth) || usernameSaving}
                  maxLength={32}
                  autoComplete="username"
                  aria-label={t("newUi.sidebar.userProfile.usernameLabel")}
                  onChange={(event) => setUsernameDraft(event.target.value)}
                  onKeyDown={(event) => {
                    if (event.key === "Enter") void handleUsernameUpdate();
                  }}
                ...
                >
                  {usernameSaving
                    ? t("common.loading")
                    : t("newUi.sidebar.userProfile.usernameSave")}
                </Button>
              </div>
              <span className="mt-1 text-[10px] leading-relaxed text-muted-foreground">
                {isOidc && !isDualAuth
                  ? t("newUi.sidebar.userProfile.usernameManagedBySso")
                  : t("newUi.sidebar.userProfile.usernameHint")}
              </span>
            </div>
            <div className="flex flex-col py-2">
              <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-semibold">
          ...
          <button
            type="button"
            className="w-full border-t border-border pt-3 mt-1 text-left enabled:cursor-pointer enabled:hover:bg-muted/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-brand/60 disabled:cursor-default"
      ...
      />
      <MfaStepUpDialog
        open={passkeyStepUpOpen}
        methods={passkeyStepUpMethods}
        onVerified={finishPasskeyStepUp}
        onCancel={() => {
          pendingPasskeyActionRef.current = null;
          setPasskeyStepUpOpen(false);
        }}
