...
export function TabBar({
  ...
  onPinSession?: (tabId: string) => void;
  pinningTabIds?: Set<string>;
    ...
    <div className="flex flex-col shrink-0 min-w-0">
      ...
      >
        ...
        >
          {tabs.map((tab, index) => {
            ...
            const pinPending = pinningTabIds?.has(tab.id) ?? false;
            const pinUnavailable =
              !tab.persistentSessionId ||
              Boolean(tab.joinSharedSessionId) ||
              Boolean(tab.agentSessionId) ||
              Boolean(tab.host?.id.startsWith("quick-connect-"));
            let translateX = 0;
            if (
              dragTabId &&
              !isDragging &&
            ...
            return (
              ...
              >
                ...
                {tab.type !== "dashboard" && renamingTabId === tab.id ? (
                  ...
                  tab.type !== "dashboard" && (
                    <span className="max-w-[4.5rem] truncate sm:max-w-32 md:max-w-48">
                      {tab.label}
                    </span>
                  )
                )}
                {tab.type === "terminal" &&
                  ...
                  renamingTabId !== tab.id && (
                    <Pin
                      className="size-3 shrink-0 text-accent-brand group-hover/tab:hidden"
                      aria-label={t("terminal.pinnedWindow")}
                    />
                  )}
                {tab.type !== "dashboard" && renamingTabId !== tab.id && (
                  <div
                  ...
                  >
                    {tab.type === "terminal" && onPinSession && (
                      <button
                        onPointerDown={(e) => e.stopPropagation()}
                        onClick={(e) => {
                          e.stopPropagation();
                          if (!pinPending && !tab.sessionPinned)
                            onPinSession(tab.id);
                        }}
                        disabled={
                          pinPending || pinUnavailable || tab.sessionPinned
                        }
                        aria-disabled={
                          pinPending || pinUnavailable || tab.sessionPinned
                        }
                        aria-pressed={tab.sessionPinned ?? false}
                        aria-label={t(
                          tab.sessionPinned
                            ? "terminal.pinnedWindow"
                            : "terminal.pinWindow",
                        )}
                        title={
                          tab.sessionPinned
                            ? t("terminal.pinnedWindowCloseOnly")
                            : pinUnavailable && !pinPending
                              ? t(
                                  tab.agentSessionId
                                    ? "terminal.agentSessionControlled"
                                    : tab.host?.id.startsWith("quick-connect-")
                                      ? "terminal.saveBeforePinning"
                                      : "terminal.sessionNotReadyToPin",
                                )
                              : t("terminal.pinWindow")
                        }
                        className={`flex size-8 shrink-0 items-center justify-center rounded-sm transition-colors md:size-5 ${
                          tab.sessionPinned
                            ? "text-accent-brand hover:bg-accent-brand/10"
                            : "text-muted-foreground hover:bg-muted hover:text-foreground"
                        } ${pinUnavailable && !pinPending ? "opacity-55" : ""} disabled:cursor-not-allowed disabled:opacity-40`}
                      >
                        {pinPending ? (
                    ...
                    {CONNECTION_TAB_TYPES.includes(tab.type) && (
                      <button
                        ...
                        className="flex size-8 items-center justify-center rounded-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground md:size-4"
                      >
                    ...
                    <button
                      ...
                      disabled={pinPending}
                      className="flex size-8 items-center justify-center rounded-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-35 md:size-4"
                    >
                      <X className="size-3" />
                    </button>
                  </div>
              ...
              {tabs.map((tab) => (
                ...
                >
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    {tab.type === "terminal" && tab.sessionPinned && (
                  ...
                  {tab.type !== "dashboard" && (
                    <button
                      ...
                      disabled={pinningTabIds?.has(tab.id)}
                      className="ml-2 shrink-0 text-muted-foreground hover:text-foreground disabled:cursor-not-allowed disabled:opacity-35"
                    >
                      <X className="size-3" />
                    </button>
                  )}
      ...
      {contextTabId &&
        ...
        (() => {
          ...
          const pinPending = pinningTabIds?.has(contextTabId) ?? false;
          const pinUnavailable =
            !ctxTab.persistentSessionId ||
            Boolean(ctxTab.joinSharedSessionId) ||
            Boolean(ctxTab.host?.id.startsWith("quick-connect-"));
          return (
            <div
              data-context-menu
              style={{
            ...
            >
              ...
              {ctxTab.type === "terminal" && onPinSession && (
                <button
                  className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs hover:bg-accent hover:text-accent-foreground disabled:cursor-not-allowed disabled:opacity-40"
                  disabled={
                    pinPending || pinUnavailable || ctxTab.sessionPinned
                  }
                  aria-disabled={
                    pinPending || pinUnavailable || ctxTab.sessionPinned
                  }
                  onClick={() => {
                    onPinSession(contextTabId);
                    setContextTabId(null);
                  }}
                >
                  {pinPending ? (
                    ...
                    <Pin
                      className={`size-3 ${ctxTab.sessionPinned ? "text-accent-brand" : ""}`}
                    />
                  )}
                  {t(
                    ctxTab.sessionPinned
                      ? "terminal.pinnedWindowCloseOnly"
                      : "terminal.pinWindow",
                  )}
                </button>
              )}
              ...
              <div className="h-px bg-border my-1" />
              <button
                className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs text-destructive hover:bg-accent hover:text-accent-foreground disabled:cursor-not-allowed disabled:opacity-35"
                disabled={pinPending}
                onClick={() => {
