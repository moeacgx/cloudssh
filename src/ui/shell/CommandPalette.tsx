...
import { createHostManagerEditEvent } from "@/sidebar/HostManagerData";
...
export function CommandPalette({
  ...
  return (
    ...
    >
      <div
        className={cn(
          // 约束整个弹窗高度，给结果列表一个确定的可滚动空间。
          // `min-h-0` 很重要：Command 是 flex 容器，默认的 min-height:auto
          // 会让列表按内容撑开，导致外层 overflow-hidden 截掉滚动区域。
          "flex h-[min(80vh,720px)] w-full max-w-2xl mx-4 max-h-[calc(100dvh-2rem)] min-h-0 overflow-hidden rounded-none border border-border bg-card shadow-2xl animate-in zoom-in-95 duration-200",
        )}
        onClick={(e) => e.stopPropagation()}
      >
        <Command className="rounded-none min-h-0 max-h-[calc(100dvh-2rem)]">
          ...
          <CommandList
            className={cn(
              // 弹窗本身有确定高度，列表占据剩余空间并独立滚动。
              "min-h-0 flex-1 max-h-none overflow-x-hidden overflow-y-auto overscroll-contain touch-pan-y thin-scrollbar",
            )}
          >
            <CommandGroup
                                  ...
                                  onClick={(e) => {
                                    ...
                                    setTimeout(() => {
                                      window.dispatchEvent(
                                        createHostManagerEditEvent(host),
                                      );
                                    }, 100);
                                  }}
                                  className="flex items-center justify-center size-7 rounded text-muted-foreground/50 hover:text-foreground hover:bg-muted-foreground/10 transition-colors"
            ...
            <CommandGroup heading={t("commandPalette.links")} className="px-2">
              <div className="grid grid-cols-1 gap-1">
                <CommandItem
                  onSelect={() =>
                    window.open("https://github.com/moeacgx/cloudssh", "_blank")
                  }
