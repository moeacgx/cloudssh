...
import { ProjectHostProvider, useProjectHostId } from "./ProjectHostContext";
...
function HostMetricsInner({
  ...
  const viewerSessionIdRef = React.useRef<string | null>(null);

  const rememberViewerSession = React.useCallback((sessionId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.viewerSessionId) {
          rememberViewerSession(result.viewerSessionId);
        }
      } else {
  ...
  React.useEffect(() => {
    ...
    const startMetrics = async () => {
      ...
      try {
        ...
        while (retry < maxRetries && !cancelled) {
          try {
            data = await getServerMetricsById(
              currentHostConfig.id,
              projectHostId,
            );
            if (data) break;
        ...
        pollingId = window.setInterval(async () => {
          if (cancelled) return;
          try {
            const next = await getServerMetricsById(
              currentHostConfig.id,
              projectHostId,
            );
            if (!cancelled && next) {
    ...
    const stopMetrics = async () => {
      ...
      const activeViewerSessionId = viewerSessionIdRef.current;
      if (currentHostConfig?.id && activeViewerSessionId) {
        viewerSessionIdRef.current = null;
        setViewerSessionId((current) =>
          current === activeViewerSessionId ? null : current,
        );
        await stopMetricsPolling(
          currentHostConfig.id,
...
export function HostMetricsTab(props: HostMetricsProps): React.ReactElement {
  return (
    <ProjectHostProvider projectHostId={props.hostConfig?.projectHostId}>
      <ConnectionLogProvider>
