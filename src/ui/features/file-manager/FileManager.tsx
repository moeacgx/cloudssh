...
function FileManagerContent({
  ...
  const connectionSessionIdRef = useRef(
    `file:${currentHost?.id ?? 0}:${currentHost?.projectHostId ?? 0}:${nanoid()}`,
  );
  ...
  async function initializeSSHConnection() {
    ...
    try {
      ...
      const sessionId = connectionSessionIdRef.current;

      const result = await connectSSH(sessionId, {
        hostId: currentHost.id,
        projectHostId: currentHost.projectHostId,
        ip: currentHost.ip,
      ...
      if (sshError.connectionLogs) {
        ...
        if (sshError.requires_totp) {
          setTotpRequired(true);
          setTotpSessionId(
            sshError.sessionId || connectionSessionIdRef.current,
          );
          setTotpPrompt(
  ...
  async function handleAuthDialogSubmit(credentials: {
    ...
    try {
      ...
      const result = await connectSSH(sessionId, {
        ...
        credentialOverride: credentials.password ? "password" : "key",
        credentialId: currentHost.credentialId,
        userId: currentHost.userId,
        jumpHosts: currentHost.jumpHosts,
        useSocks5: currentHost.useSocks5,
