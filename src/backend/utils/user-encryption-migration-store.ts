  ...
  updateHostSensitiveFields(
    ...
    this.db
      .prepare(
        `
          UPDATE ssh_data
          SET password = ?, key = ?, key_password = ?, key_type = ?,
              autostart_password = ?, autostart_key = ?,
              autostart_key_password = ?, sudo_password = ?,
              socks5_password = ?, rdp_password = ?, vnc_password = ?,
              telnet_password = ?, tunnel_connections = ?,
              terminal_config = ?, socks5_proxy_chain = ?,
      ...
      .run(
        ...
        record.socks5_password || null,
        record.rdp_password || null,
        record.vnc_password || null,
        record.telnet_password || null,
        record.tunnel_connections || null,
        record.terminal_config || null,
        record.socks5_proxy_chain || null,
        recordId,
      );
  }
