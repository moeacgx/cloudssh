...
import {
  projectRoleHostPermission,
  type ProjectHostAccess,
  type ProjectRole,
} from "../control-plane/project-repository.js";
...
function projectRoleAllowsHostAction(
  role: ProjectRole,
  action: HostAction,
): boolean {
  if (action === "delete") return false;
  if (role === "instance_admin" || role === "project_admin") return true;
  if (role === "operator") return action === "connect" || action === "view";
  return action === "view";
}

function projectHostAccessInfo(
  ...
  return {
    ...
    isOwner: false,
    isShared: true,
    permissionLevel: projectRoleHostPermission(access.role),
    expiresAt: null,
...
class PermissionManager {
  ...
  async canAccessHost(
    ...
    try {
      ...
      const projectRepository = createCurrentProjectRepository();

      if (projectHostId !== undefined) {
        const projectAccess = await projectRepository.findHostAccess(
          hostId,
          userId,
          projectHostId,
        );
        if (projectAccess) {
          return projectHostAccessInfo(
            projectAccess,
            projectRoleAllowsHostAction(projectAccess.role, action),
          );
        }
        if (await this.isAdmin(userId)) return this.adminBypassAccess();
        return {
          hasAccess: false,
          isOwner: false,
          isShared: false,
      ...
      if (
        (await hostResolutionRepository.isHostOwnedByUser(hostId, userId)) &&
        (await projectRepository.isHostOwnerControlled(hostId, userId))
      ) {
        return {
          hasAccess: true,
          isOwner: true,
      ...
      let deniedLegacyAccess: HostAccessInfo | null = null;
      if (access) {
      ...
      if (
        ...
        return projectHostAccessInfo(projectAccess, true);
      }
      ...
      if (projectAccess) return projectHostAccessInfo(projectAccess, false);
      if (deniedLegacyAccess) return deniedLegacyAccess;
