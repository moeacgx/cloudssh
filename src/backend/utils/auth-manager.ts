...
import { logAuditOrThrow, getRequestMeta } from "./audit-logger.js";
...
class AuthManager {
  ...
  async generateJWTToken(
    ...
    if (
      !options.pendingTOTP &&
      Number.isSafeInteger(options.mfaVerifiedAt) &&
      options.mfaVerifiedAt! > 0
    ) {
      payload.mfaVerifiedAt = options.mfaVerifiedAt;
    }

    if (!options.pendingTOTP && options.deviceType && options.deviceInfo) {
      payload.authVerifiedAt =
        Number.isSafeInteger(options.authVerifiedAt) &&
        options.authVerifiedAt! > 0
          ? options.authVerifiedAt
  ...
  async refreshSessionToken(
    userId: string,
    sessionId: string,
    options: { mfaVerifiedAt?: number; authVerifiedAt?: number } = {},
    ...
    if (
      ...
      payload.authVerifiedAt = options.authVerifiedAt;
    }

    try {
      const previous = jwt.verify(
        sessionRecord.jwtToken,
        await this.systemCrypto.getJWTSecret(),
      ) as JWTPayload;
      if (
        payload.mfaVerifiedAt === undefined &&
        previous.userId === userId &&
        previous.sessionId === sessionId &&
        Number.isSafeInteger(previous.mfaVerifiedAt) &&
        previous.mfaVerifiedAt! > 0
      ) {
        payload.mfaVerifiedAt = previous.mfaVerifiedAt;
      }
      if (
        payload.authVerifiedAt === undefined &&
        previous.userId === userId &&
        previous.sessionId === sessionId &&
        Number.isSafeInteger(previous.authVerifiedAt) &&
        previous.authVerifiedAt! > 0
      ) {
        payload.authVerifiedAt = previous.authVerifiedAt;
      }
    } catch {
      // 当前会话已经由仓储校验；旧令牌异常时仅不延续 MFA 提权状态。
    }
  ...
  createAuthMiddleware() {
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      authReq.authVerifiedAt = payload.authVerifiedAt;
  ...
  private async applyAdminImpersonation(
    ...
    if (!isAdministrativeTransportAllowed(req)) {
      databaseLogger.warn("Insecure administrator impersonation denied", {
        operation: "admin_impersonation_https_required",
        userId: adminUserId,
        targetUserId,
        path,
      });
      res.status(426).json({
        error: "HTTPS is required for administrator access to user data",
        code: "HTTPS_REQUIRED",
      });
      return true;
    }
    ...
    try {
      ...
      databaseLogger.error("Impersonation audit write failed", error, {
        operation: "admin_impersonation_audit_failed",
        userId: adminUserId,
        targetUserId,
        path,
      });
