...
export function registerDockerSshRoutes(app: express.Express): void {
  ...
  app.post("/docker/ssh/connect", async (req, res) => {
    ...
    if (
      !isValidDockerSessionId(sessionId) ||
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Invalid projectHostId", connectionLogs });
    }
    ...
    try {
      const hostRecord = await resolveHostById(hostId, userId, projectHostId);
      ...
      if (
        (sshSessions[sessionId]?.userId &&
          sshSessions[sessionId].userId !== userId) ||
        (pendingTOTPSessions[sessionId]?.userId &&
