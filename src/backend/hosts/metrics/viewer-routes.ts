...
>(
  ...
  app.post("/metrics/register-viewer", async (req, res) => {
    ...
    try {
      ...
      try {
        host = await fetchHostById(hostId, userId, projectHostId);
      } catch (lookupErr) {
        statsLogger.warn(
  ...
  app.post("/metrics/unregister-viewer", async (req, res) => {
    ...
    try {
      if (!unregisterViewer(hostId, viewerSessionId, userId, projectHostId)) {
        return res.status(404).json({ error: "Viewer session not found" });
      }
      res.json({ success: true });
    } catch (error) {
