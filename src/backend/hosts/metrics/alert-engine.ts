...
export class AlertEngine {
  ...
  // "${userId}:${hostId}" → last known status
  private lastStatusMap = new Map<string, "online" | "offline">();
  ...
  async evaluateStatus(
    ...
    const stateKey = `${userId}:${hostId}`;
    const lastStatus = this.lastStatusMap.get(stateKey);

    if (lastStatus === currentStatus) return;
    this.lastStatusMap.set(stateKey, currentStatus);
