...
/**
 * Agent 会话由 Agent Broker 独占管理，普通 tmux 监控接口不能修改它们。
 * 这里统一在执行远端命令前做服务端数据库归属校验，避免仅依赖前端隐藏
 * 列表导致窗口名称被猜中后仍可被重命名、杀掉或创建子窗口。
 */
function rejectAgentManagedSession(
  res: express.Response,
  host: SSHHost,
  sessionName: string,
): boolean {
  if (!isAgentManagedTmuxSession(sessionName, host.id)) return false;
  res.status(409).json({
    error: "Agent 持续会话由 Agent 管理，不能通过普通 tmux 监控接口操作",
...
/**
 * 根据 pane ID 解析所属 session。pane ID 本身不包含 session 名称，因而
 * 对 focus/split/kill-pane 这类接口必须在远端查询后再应用 Agent 策略。
 * 查询失败必须向上抛出；不能把“无法确认归属”当成普通 pane，从而放行
 * focus/split/kill-pane 等破坏性操作。
 */
async function resolvePaneSessionName(
  conn: Client,
  paneId: string,
): Promise<string | null> {
  const output = await execCommand(
    conn,
    tmuxCommand(
      `display-message -p -t ${shellEscape(paneId)} "#{session_name}" 2>/dev/null`,
    ),
  );
...
async function rejectAgentManagedPane(
  res: express.Response,
  host: SSHHost,
  conn: Client,
  paneId: string,
): Promise<boolean> {
  const sessionName = await resolvePaneSessionName(conn, paneId);
  if (!sessionName) {
    res.status(404).json({ error: "Pane not found" });
    return true;
  }
  return rejectAgentManagedSession(res, host, sessionName);
}
...
app.get("/tmux_monitor/:hostId/overview", async (req, res) => {
  ...
  try {
    const result = await withHostConnection(host, async (conn) => {
      ...
      const sessions = hideAgentManagedTmuxSessions(
        parseSessions(sessionsOut),
        host.id,
      );
...
app.post("/tmux_monitor/:hostId/focus", async (req, res) => {
  ...
  try {
    const rejected = await withHostConnection(host, async (conn) => {
      if (await rejectAgentManagedPane(res, host, conn, paneId)) return true;
      // pane ID 是合法的窗口目标：tmux 会将其解析为包含该 pane 的窗口。
      await execCommand(
        conn,
        tmuxCommand(
...
app.post("/tmux_monitor/:hostId/sessions", async (req, res) => {
  ...
  if (rejectAgentManagedSession(res, host, name)) return;
...
app.post("/tmux_monitor/:hostId/windows", async (req, res) => {
  ...
  if (rejectAgentManagedSession(res, host, sessionName)) return;
...
app.post("/tmux_monitor/:hostId/kill-pane", async (req, res) => {
  ...
  try {
    const rejected = await withHostConnection(host, async (conn) => {
      ...
      await execCommand(
        conn,
        tmuxCommand(`kill-pane -t ${shellEscape(paneId)}`),
      );
      return false;
    });
    if (rejected) return;
...
app.get("/tmux_monitor/:hostId/search", async (req, res) => {
  ...
  try {
    const results = await withHostConnection(host, async (conn) => {
      const visiblePanes = (await listPanesRaw(conn)).filter(
        (pane) => !isAgentManagedTmuxSession(pane.sessionName, host.id),
      );
      const panes = visiblePanes.slice(0, MAX_SEARCH_PANES);
      // Flips to true whenever a limit was hit, so the UI can tell the user
      // the results are partial instead of silently truncating.
      let truncated = visiblePanes.length > MAX_SEARCH_PANES;
      const matches: Array<{
        paneId: string;
        sessionName: string;
        windowIndex: number;
...
app.get("/tmux_monitor/:hostId/metrics", async (req, res) => {
  ...
  try {
    const metrics = await withHostConnection(host, async (conn) => {
      const panes = (await listPanesRaw(conn)).filter(
        (pane) => !isAgentManagedTmuxSession(pane.sessionName, host.id),
      );
