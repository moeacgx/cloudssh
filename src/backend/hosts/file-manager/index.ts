...
import {
  ...
  type FileManagerConnectionCredentials,
...
const sshSessions: Record<string, SSHSession> = Object.create(null);
...
const activeListRequests: Record<string, boolean> = Object.create(null);

function cleanupSession(sessionId: string, force = false) {
  const session = sshSessions[sessionId];
  if (session) {
    if (!force && session.activeOperations > 0) {
      fileLogger.warn(
...
function verifySessionOwnership(session: SSHSession, userId: string): boolean {
  return isFileManagerSessionOwner(session.userId, userId);
}
...
function resolveBrowseHostId(
  ...
  return parseFileManagerSessionContext(browseSessionId)?.hostId;
}
...
async function openDedicatedTransferSession(
  ...
  const encodedContext = parseFileManagerSessionContext(browseSessionId);
  const hostId = browseSession
    ? resolveBrowseHostId(browseSessionId, browseSession)
    : encodedContext?.hostId;
  if (!hostId) {
  ...
  const host = await resolveHostById(
    hostId,
    userId,
    browseSession?.projectHostId ?? encodedContext?.projectHostId,
  );
  if (!host) {
...
app.use("/ssh/file_manager/ssh", (req, res, next) => {
  ...
  if (sessionId !== undefined && !isValidFileManagerSessionId(sessionId)) {
...
app.post("/ssh/file_manager/ssh/connect", async (req, res) => {
  ...
  if (
    !isValidFileManagerSessionId(sessionId) ||
    !ip ||
    !username ||
    !port ||
    (projectHostId !== undefined &&
      (!Number.isSafeInteger(projectHostId) || projectHostId <= 0))
  ) {
  ...
  const encodedSessionContext = parseFileManagerSessionContext(sessionId);
  if (
    encodedSessionContext &&
    (encodedSessionContext.hostId !== Number(hostId) ||
      encodedSessionContext.projectHostId !== projectHostId)
  ) {
    return res.status(400).json({
      error: "Session context does not match the requested host",
      code: "SESSION_CONTEXT_MISMATCH",
      connectionLogs,
    });
  }

  const existingSession = sshSessions[sessionId];
  if (existingSession && !verifySessionOwnership(existingSession, userId)) {
  ...
  const pendingSession = pendingTOTPSessions[sessionId];
  if (
    pendingSession &&
    !isFileManagerSessionOwner(pendingSession.userId, userId)
  ) {
    return res.status(409).json({
      error: "Session ID is already in use",
      code: "SESSION_ID_CONFLICT",
  ...
  // 已保存主机始终从服务端解析。浏览器可能仍保留上一次失败时的旧密码，
  // 不能以“请求里带了秘密”为由跳过最新凭据。
  let resolvedCredentials: FileManagerConnectionCredentials = {
    password,
    sshKey,
    keyPassword,
    authType,
  ...
  const savedHost = isSavedFileManagerHost(hostId);
  if (savedHost) {
    try {
      ...
      if (resolvedHost) {
        ...
        resolvedUseSocks5 = resolvedHost.useSocks5 ?? false;
        ...
        if (resolvedJumpHosts.length > 0) {
          connectionLogs.push(
            createConnectionLog(
              "info",
              "jump",
              `Loaded ${resolvedJumpHosts.length} jump host(s) from server-side host data`,
            ),
          );
        }
        connectionLogs.push(
      ...
      return res.status(403).json({
        error: "Saved host could not be resolved",
        code: "HOST_ACCESS_DENIED",
        connectionLogs,
      });
    }
  ...
  if (savedHost) {
    try {
      resolvedCredentials = applyFileManagerCredentialOverride(
        resolvedCredentials,
        { password, sshKey, keyPassword, authType },
        credentialOverride,
      );
    } catch (error) {
      return res.status(400).json({
  ...
  client.on("close", () => {
    ...
    const currentSession = sshSessions[sessionId];
...
app.post("/ssh/file_manager/ssh/connect-totp", async (req, res) => {
  ...
  if (!isFileManagerSessionOwner(session.userId, userId)) {
    return res.status(403).json({ error: "Session access denied" });
  }
...
shutdownCleanupRegistry.register("file-manager", async () => {
  if (server) await closeHttpServer(server);

  for (const sessionId of Object.keys(sshSessions)) {
