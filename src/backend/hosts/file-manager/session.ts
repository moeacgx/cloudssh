...
const FILE_MANAGER_SESSION_ID_PATTERN = /^[A-Za-z0-9._:-]{1,128}$/;
const RESERVED_SESSION_IDS = new Set(["__proto__", "prototype", "constructor"]);

export function isValidFileManagerSessionId(value: unknown): value is string {
  return (
    typeof value === "string" &&
    FILE_MANAGER_SESSION_ID_PATTERN.test(value) &&
    !RESERVED_SESSION_IDS.has(value)
  );
}

export function parseFileManagerSessionContext(value: string): {
  hostId: number;
  projectHostId?: number;
} | null {
  const encoded = value.match(/^file:(\d+):(\d+):[A-Za-z0-9_-]+$/);
  if (encoded) {
    const hostId = Number(encoded[1]);
    const projectHostId = Number(encoded[2]);
    if (!Number.isSafeInteger(hostId) || hostId <= 0) return null;
    return {
      hostId,
      projectHostId:
  ...
  const legacyHostId = Number(value);
  return Number.isSafeInteger(legacyHostId) && legacyHostId > 0
    ? { hostId: legacyHostId }
    : null;
}

export function isFileManagerSessionOwner(
  ownerUserId: string | undefined,
  requestUserId: string,
): boolean {
  return !ownerUserId || ownerUserId === requestUserId;
}
