...
export function generateEphemeralKeyPair(
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (
      !(ssh2Utils.parseKey(pair.private) instanceof Error) &&
      !(ssh2Utils.parseKey(pair.public) instanceof Error)
    ) {
      return { privateKey: pair.private, publicKey: pair.public };
    }
  }
  throw new Error(`Failed to generate a parseable ${ssh2Type} SSH keypair`);
}
