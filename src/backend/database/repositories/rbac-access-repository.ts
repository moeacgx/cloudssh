...
export class RbacAccessRepository {
  ...
  async listVisibleHostAccessEntries(
    ...
    const legacyEntries = await this.context.drizzle
      .select({
    ...
    let projectEntries: RbacVisibleHostAccessEntry[] = [];
    try {
      projectEntries = (
        await new ProjectRepository(this.context).listAccessibleHostEntries(
          userId,
        )
      ).map((entry) => ({
        hostId: entry.hostId,
        permissionLevel: projectRoleHostPermission(entry.role),
        expiresAt: null,
      }));
    } catch (error) {
      databaseLogger.warn("Failed to load project-based host access", {
        operation: "project_host_access_list_failed",
        userId,
        error: error instanceof Error ? error.message : "Unknown error",
      });
    }

    const strongestByHost = new Map<number, RbacVisibleHostAccessEntry>();
    for (const entry of [...legacyEntries, ...projectEntries]) {
      const current = strongestByHost.get(entry.hostId);
      if (
        !current ||
        (HOST_PERMISSION_RANK[entry.permissionLevel] ?? 0) >
          (HOST_PERMISSION_RANK[current.permissionLevel] ?? 0)
      ) {
        strongestByHost.set(entry.hostId, entry);
      }
    }
