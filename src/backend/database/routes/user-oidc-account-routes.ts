...
export function registerUserOidcAccountRoutes(
  router: Router,
  { authenticateJWT }: UserOidcAccountRoutesDeps,
): void {
  /**
   * @openapi
  ...
  router.post("/link-oidc-to-password", authenticateJWT, async (req, res) => {
    ...
    try {
      ...
      // 目标账号更新、成员关系迁移和源账号删除必须在同一个事务中完成。
      await mergeOidcUserIntoPasswordUser(oidcUserId, targetUser.id);
      ...
      if (
        ...
        err instanceof UserOwnsSharedResourcesError ||
        err instanceof UserMergeHasOwnedDataError
      ) {
