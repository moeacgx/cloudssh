...
import {
  ...
  synchronizeProjectHostCredentialsForOwner,
...
type AdminCredentialSecretField = "password" | "key" | "keyPassword";

function credentialPrivateKey(
  credential: Record<string, unknown>,
): string | null {
  const value = credential.privateKey || credential.key;
  return typeof value === "string" && value.length > 0 ? value : null;
}

function disclosedCredentialFields(
  credential: Record<string, unknown>,
): AdminCredentialSecretField[] {
  const fields: AdminCredentialSecretField[] = [];
  if (typeof credential.password === "string" && credential.password) {
    fields.push("password");
  }
  if (credentialPrivateKey(credential)) fields.push("key");
  if (typeof credential.keyPassword === "string" && credential.keyPassword) {
    fields.push("keyPassword");
  }
  return fields;
}

async function auditAdminCredentialSecretAccess(
  req: AuthenticatedRequest,
  credential: Record<string, unknown>,
  action:
    | "admin_view_user_credential_secret"
    | "admin_copy_user_credential_secret",
  fields: AdminCredentialSecretField[],
): Promise<void> {
  ...
  await logAuditOrThrow({
    ...
    resourceName: String(credential.name ?? credential.id ?? ""),
...
async function resyncProjectCredentialsForOwner(
  ownerId: string,
  operation: string,
): Promise<void> {
  try {
    await synchronizeProjectHostCredentialsForOwner(ownerId);
  } catch (error) {
    authLogger.warn("Failed to resync project credentials", {
      operation,
      ownerId,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (
      authReq.actingAdminUserId &&
      !requireSecureRecentInteractiveMfa(authReq, res)
    ) {
      return;
    }
    ...
    try {
      ...
      const privateKey = credentialPrivateKey(credential);
      output.hasKey = !!privateKey;
      ...
      if (authReq.actingAdminUserId) {
        try {
          await auditAdminCredentialSecretAccess(
            authReq,
            credential,
            "admin_view_user_credential_secret",
            disclosedCredentialFields(credential),
          );
        } catch (auditError) {
          authLogger.error("Admin credential view audit failed", auditError, {
            operation: "admin_credential_secret_view_audit_failed",
...
/**
 * 返回管理员明确请求复制的单个凭据秘密。管理员身份由代管中间件校验，
 * 审计写入失败时拒绝返回明文。
 */
router.get(
  "/:id/admin-secret",
  ...
  async (req: Request, res: Response) => {
    ...
    if (field !== "password" && field !== "key" && field !== "keyPassword") {
    ...
    try {
      ...
      const value =
        field === "key" ? credentialPrivateKey(credential) : credential[field];
      if (typeof value !== "string" || value.length === 0) {
        return res.status(404).json({ error: "Credential secret not found" });
      }
      ...
      try {
        ...
        authLogger.error("Admin credential copy audit failed", auditError, {
          operation: "admin_credential_secret_copy_audit_failed",
          adminUserId: authReq.actingAdminUserId,
      ...
      authLogger.error(
        "Failed to copy administrator credential secret",
        error,
        {
          operation: "admin_credential_secret_copy",
          adminUserId: authReq.actingAdminUserId,
          targetUserId: authReq.userId,
          credentialId,
          field,
      ...
      return res
        .status(500)
        .json({ error: "Failed to load credential secret" });
    }
  },
);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      await resyncProjectCredentialsForOwner(
        userId,
        "credential_update_project_resync",
      );
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      await resyncProjectCredentialsForOwner(
        userId,
        "credential_delete_project_resync",
      );
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        authLogger.warn("Failed to resync applied project credential", {
          operation: "credential_apply_project_resync",
          ownerId: userId,
