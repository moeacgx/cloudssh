...
export function isMissingRequiredSshPassword(
  connectionType: unknown,
  authType: unknown,
  password: unknown,
): boolean {
  return (
    (connectionType === undefined || connectionType === "ssh") &&
    authType === "password" &&
...
const NESTED_SECRET_KEYS = new Set([
  "apikey",
  "accesstoken",
  "autostartkey",
  "autostartkeypassword",
...
export function redactNestedSecrets(value: unknown, depth = 0): unknown {
  if (depth > 32) return null;
  if (Array.isArray(value)) {
    return value.map((entry) => redactNestedSecrets(entry, depth + 1));
  }
  if (!value || typeof value !== "object") return value;

  const result: Record<string, unknown> = {};
  for (const [key, nestedValue] of Object.entries(value)) {
    if (NESTED_SECRET_KEYS.has(normalizedSecretKey(key))) continue;
    result[key] = redactNestedSecrets(nestedValue, depth + 1);
  }
  return result;
}

export function parseAndRedactNestedHostConfig(
  value: unknown,
  fallback: unknown,
): unknown {
  if (value === null || value === undefined || value === "") return fallback;
  if (typeof value !== "string") return redactNestedSecrets(value);
  try {
    return redactNestedSecrets(JSON.parse(value));
  } catch {
    return fallback;
  }
}
...
const NETWORK_INFO_STORAGE_FIELDS = [
  "networkInfoStatus",
  "networkLookupSource",
  "networkResolvedIp",
  "networkCountryCode",
...
export function sanitizeHostForRecipient(
  ...
  for (const field of NESTED_CONFIG_FIELDS) {
    if (stripped[field] !== undefined) {
      stripped[field] = parseAndRedactNestedHostConfig(
        stripped[field],
        Array.isArray(stripped[field]) ? [] : null,
      );
    }
  }
  delete stripped.portKnockSequence;
...
export function transformHostResponse(
  ...
  const result: Record<string, unknown> = {
    ...host,
    networkInfo: hostNetworkInfoFromRecord(host),
    tags:
    ...
    socks5ProxyChain: host.socks5ProxyChain
      ...
      : [],
    portKnockSequence: normalizePortKnockSequence(host.portKnockSequence),
  ...
  // 数据库缓存字段只用于构造稳定的 networkInfo API。尤其在地址刚修改时，
  // 不能让旧地址、地区或 ISP 从展开的原始行中旁路泄露给任何权限级别。
  for (const field of NETWORK_INFO_STORAGE_FIELDS) delete result[field];
  return result;
}
