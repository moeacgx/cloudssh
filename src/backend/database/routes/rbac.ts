...
import {
  ...
  type RoleTerminalLifecycleTarget,
...
function hasPinnedSessionsForRoleTarget(
  target: RoleTerminalLifecycleTarget,
): boolean {
  const projectSessions =
    target.projectHostIds.length > 0
      ? sessionManager.findSessions({
          projectHostIds: target.projectHostIds,
          userIds: target.userIds,
          pinned: true,
        })
      : [];
  if (projectSessions.length > 0) return true;

  return (
    target.hostIds.length > 0 &&
    sessionManager.findSessions({
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      const roleRepository = createCurrentRoleRepository();
      const lifecycleTarget = roleRepository.getTerminalLifecycleTarget(roleId);
      const { deletedUserIds } =
        await terminalSessionLifecycleCoordinator.runDestructiveOperation(
          lifecycleTarget,
          async () => {
            if (hasPinnedSessionsForRoleTarget(lifecycleTarget)) {
              throw new RoleHasPinnedTerminalSessionsError(1);
            }
      ...
      if (error instanceof RoleHasPinnedTerminalSessionsError) {
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      const actor = await createCurrentUserRepository().findById(currentUserId);
      const requestMeta = getRequestMeta(req);
      try {
        await logAuditOrThrow({
          userId: currentUserId,
          username: actor?.username ?? currentUserId,
          action: "assign_user_role_intent",
          resourceType: "user_role",
          resourceId: `${targetUserId}:${roleId}`,
          resourceName: `${targetUser.username}:${role.name}`,
        ...
        databaseLogger.error("Role assignment intent audit failed", error, {
          operation: "assign_role_audit_failed",
          currentUserId,
          targetUserId,
          roleId,
        });
    ...
    try {
      ...
      try {
        await logAuditOrThrow({
          userId: currentUserId,
          username: actor?.username ?? currentUserId,
          action: "remove_user_role_intent",
          resourceType: "user_role",
          resourceId: `${targetUserId}:${roleId}`,
          resourceName: `${targetUser.username}:${role.name}`,
          details: JSON.stringify({
        ...
        databaseLogger.error("Role removal intent audit failed", error, {
          operation: "remove_role_audit_failed",
          currentUserId,
          targetUserId,
          roleId,
        });
      ...
      const roleRepository = createCurrentRoleRepository();
      const lifecycleTarget = roleRepository.getTerminalLifecycleTarget(
        roleId,
        targetUserId,
      );
      await terminalSessionLifecycleCoordinator.runDestructiveOperation(
        ...
        async () => {
          ...
          await roleRepository.removeRoleFromUser(targetUserId, roleId, {
            rejectPinnedTerminalSessions: true,
          });
        },
      );
