...
import { validateUsername } from "./user-username-utils.js";
  ...
  try {
    ...
    if (config.admin_group) {
      ...
      if (!!userRecord.isAdmin !== shouldBeAdmin) {
        ...
        const synchronized =
          await createCurrentRoleRepository().setUserAdminStatus({
            userId: userRecord.id,
            isAdmin: shouldBeAdmin,
            grantedBy: userRecord.id,
          });
        if (!synchronized) {
          throw new Error("OIDC user disappeared during admin role sync");
        }
        userRecord = { ...userRecord, isAdmin: shouldBeAdmin };
 ...
 * /users/me/username:
 *   patch:
 *     summary: Change the current user's login username
 *     tags:
 *       - Users
 */
router.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (user.isOidc && !hasPassword) {
        return res.status(403).json({
          error: "Username is managed by the identity provider",
          code: "USERNAME_MANAGED_BY_SSO",
        });
      }

      if (user.username === validation.username) {
        return res.json({ success: true, username: user.username });
      }

      const oldUsername = user.username;
      const result = await userRepository.updateUsernameIfAvailable(
        userId,
        validation.username,
      );
      ...
      authLogger.success("User changed username", {
        operation: "user_change_username",
        userId,
        oldUsername,
        username: result.user.username,
      });
      ...
      authLogger.error("Failed to change username", err, {
        operation: "user_change_username_failed",
        userId,
      });
      return res.status(500).json({ error: "Failed to change username" });
    }
...
router.delete("/delete-account", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (
      err instanceof UserHasPinnedTerminalsError ||
      err instanceof UserHasActiveTerminalsError
    ) {
      return res.status(409).json({
        error: err.message,
        code: err.code,
    ...
    if (err instanceof UserOwnsSharedResourcesError) {
      return res.status(409).json({
        error: err.message,
        code: err.code,
        teamCount: err.teamCount,
        teamProjectCount: err.teamProjectCount,
        sharedHostCount: err.sharedHostCount,
        activePersistentSessionCount: err.activePersistentSessionCount,
        retainedTeamRecordingCount: err.retainedTeamRecordingCount,
      });
    }
