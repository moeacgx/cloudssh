  ...
  router.post("/make-admin", authenticateJWT, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        authLogger.error(
          "Failed to persist admin promotion intent",
          auditError,
          {
            operation: "make_admin_audit_failed",
      ...
      if (
        !(await roleRepository.setUserAdminStatus({
          userId: targetUser.id,
          isAdmin: true,
          grantedBy: userId,
        }))
  ...
  router.post("/remove-admin", authenticateJWT, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        authLogger.error("Failed to persist admin removal intent", auditError, {
          operation: "remove_admin_audit_failed",
          userId: targetUser.id,
        });
        return res.status(503).json({
          error: "Audit storage is unavailable",
    ...
    try {
      ...
      if (!isAdministrativeTransportAllowed(req)) {
        return res.status(426).json({
          error: "HTTPS is required for administrator data export",
          code: "HTTPS_REQUIRED",
        });
      }

      if (!requireRecentInteractiveMfa(req as AuthenticatedRequest, res)) {
        return;
      }
      ...
      try {
        await logAuditOrThrow({
          ...auditBase,
          action: "admin_export_user_data_intent",
        });
      } catch (auditError) {
        authLogger.error("Admin export intent audit failed", auditError, {
          operation: "admin_export_user_data_intent_audit_failed",
          adminId,
          targetUserId: targetUser.id,
        });
        return res.status(503).json({
      ...
      try {
        ...
        authLogger.error("Admin export result audit failed", auditError, {
          operation: "admin_export_user_data_audit_failed",
          adminId,
          targetUserId: targetUser.id,
        });
        return res.status(503).json({
