...
import {
  runUserTerminalDestructiveOperation,
...
export async function resetUserPassword(
  ...
  if (authManager.isUserUnlocked(userId)) {
    await createCurrentUserDataLifecycleRepository().resetPasswordPreservingData(
      {
        userId,
        username,
        passwordHash,
  ...
  return runUserTerminalDestructiveOperation(userId, async () => {
    const { UserKeyManager } = await import("../../utils/user-keys.js");
    const userKeys = UserKeyManager.getInstance();
    const wrappedDek = userKeys.prepareUserDEKRotation(userId);

    await createCurrentUserDataLifecycleRepository().wipeEncryptedUserData({
      userId,
      username,
      passwordHash,
      wrappedDek,
    });
    userKeys.invalidate(userId);
    await authManager.logoutUser(userId);
