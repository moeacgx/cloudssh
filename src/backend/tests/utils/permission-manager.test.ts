...
vi.mock("../../control-plane/factory.js", () => ({
  createCurrentProjectRepository: () => ({
    findHostAccess: async () => accessState.projectAccess,
    isHostOwnerControlled: async () => accessState.ownerControlled,
  }),
}));
...
describe("PermissionManager.canAccessHost level hierarchy", () => {
  ...
  beforeEach(() => {
    ...
    accessState.ownerControlled = true;
    accessState.projectAccess = null;
  });
  ...
  it("does not let a storage owner bypass a team project's ACL", async () => {
    accessState.ownerControlled = false;

    await expect(
      manager.canAccessHost("owner", 42, "connect"),
    ).resolves.toMatchObject({ hasAccess: false, isOwner: false });

    accessState.projectAccess = {
    ...
    await expect(
      manager.canAccessHost("owner", 42, "view"),
  ...
  it("does not restore team-host ownership through a stale legacy grant", async () => {
    accessState.ownerControlled = false;
    accessState.grant = {
      id: 11,
    ...
    await expect(
      manager.canAccessHost("owner", 42, "manage"),
    ).resolves.toMatchObject({
      hasAccess: true,
      isOwner: false,
      isShared: true,
  ...
  it("maps project roles to host actions without separate host sharing", async () => {
    accessState.projectAccess = {
      projectId: "project-1",
      projectHostId: 9,
      hostId: 42,
    ...
    await expect(
      manager.canAccessHost("member", 42, "manage", 9),
    ).resolves.toMatchObject({
      hasAccess: true,
      permissionLevel: "manage",
      projectHostId: 9,
    ...
    await expect(
      manager.canAccessHost("member", 42, "delete", 9),
    ).resolves.toMatchObject({ hasAccess: false });

    accessState.projectAccess.role = "operator";
    await expect(
      manager.canAccessHost("member", 42, "connect", 9),
    ).resolves.toMatchObject({ hasAccess: true, permissionLevel: "connect" });
    await expect(
      manager.canAccessHost("member", 42, "edit", 9),
    ).resolves.toMatchObject({ hasAccess: false });

    accessState.projectAccess.role = "viewer";
    await expect(
      manager.canAccessHost("member", 42, "view", 9),
    ).resolves.toMatchObject({ hasAccess: true, permissionLevel: "view" });
    await expect(
      manager.canAccessHost("member", 42, "connect", 9),
    ).resolves.toMatchObject({ hasAccess: false });
  });
