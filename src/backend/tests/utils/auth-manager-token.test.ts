...
const mocks = vi.hoisted(() => ({
  ...
  updateSessionToken: vi.fn(async () => {}),
}));
...
vi.mock("../../database/repositories/factory.js", () => ({
  ...
  createCurrentSessionRepository: () => ({
    ...
    updateToken: mocks.updateSessionToken,
  }),
...
beforeEach(() => {
  ...
  mocks.updateSessionToken.mockReset().mockResolvedValue(undefined);
});
...
describe("AuthManager token handling", () => {
  ...
  it("can add a recent MFA timestamp while refreshing the current session", async () => {
    ...
    expect(
      (jwt.decode(refreshed!.token) as Record<string, unknown>).mfaVerifiedAt,
    ).toBe(verifiedAt);
    expect(mocks.updateSessionToken).toHaveBeenCalledWith(
      "session-1",
      refreshed!.token,
    );
  });
