...
describe("FieldCrypto.shouldEncryptField", () => {
  it("identifies encrypted fields per table", () => {
    ...
    expect(
      FieldCrypto.shouldEncryptField("ssh_data", "tunnelConnections"),
    ).toBe(true);
    expect(FieldCrypto.shouldEncryptField("ssh_data", "terminalConfig")).toBe(
      true,
    );
    expect(FieldCrypto.shouldEncryptField("ssh_data", "socks5ProxyChain")).toBe(
      true,
    );
    expect(
