...
describe("HostPollCache", () => {
  ...
  it("isolates simultaneous project scopes for the same host", () => {
    const cache = new HostPollCache<{ id: number; credential: string }>(60_000);
    cache.set(1, "user-a:project-10", { id: 1, credential: "project-a" });
    cache.set(1, "user-a:project-20", { id: 1, credential: "project-b" });

    expect(cache.get(1, "user-a:project-10")?.credential).toBe("project-a");
    expect(cache.get(1, "user-a:project-20")?.credential).toBe("project-b");
  });
