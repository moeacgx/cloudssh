...
describe("isMissingRequiredSshPassword", () => {
  it("rejects an SSH password host when the password is missing", () => {
    expect(isMissingRequiredSshPassword("ssh", "password", undefined)).toBe(
      true,
    );
    expect(isMissingRequiredSshPassword(undefined, "password", "   ")).toBe(
      true,
    );
  });

  it("accepts a supplied password and non-password authentication", () => {
    expect(isMissingRequiredSshPassword("ssh", "password", "secret")).toBe(
      false,
    );
    expect(isMissingRequiredSshPassword("ssh", "key", undefined)).toBe(false);
    expect(isMissingRequiredSshPassword("rdp", "password", undefined)).toBe(
      false,
    );
  });
});
...
describe("transformHostResponse", () => {
  ...
  it("normalizes port knocking arrays, JSON text, and malformed legacy values", () => {
    expect(
      transformHostResponse({
        portKnockSequence: '[{"port":4000,"protocol":"udp"}]',
      }).portKnockSequence,
    ).toEqual([{ port: 4000, protocol: "udp", delay: undefined }]);
    expect(
      transformHostResponse({
        portKnockSequence: [{ port: 5000, protocol: "tcp", delay: 100 }],
      }).portKnockSequence,
    ).toEqual([{ port: 5000, protocol: "tcp", delay: 100 }]);
    expect(
      transformHostResponse({ portKnockSequence: { port: 6000 } })
        .portKnockSequence,
    ).toEqual([]);
  });
  ...
  it("地址变化后只返回统一 networkInfo，不暴露旧缓存字段", () => {
    const result = transformHostResponse({
      ip: "1.1.1.1",
      ...
      network_lookup_source: "old-snake-case-source",
    });

    expect(result.networkInfo).toMatchObject({
      status: "unknown",
      resolvedIp: null,
      country: null,
      isp: null,
    ...
    expect(JSON.stringify(result)).not.toContain("Old ISP");
    expect(JSON.stringify(result)).not.toContain("old-snake-case-source");
  });
});
...
describe("sanitizeHostForRecipient", () => {
  ...
  it("always strips secrets for recipients", () => {
        ...
        tunnelConnections: [
          {
            ...
            auth: { privateKey: "nested-private-key", username: "root" },
          },
        ],
        terminalConfig: JSON.stringify({
          shell: "bash",
          sudoPassword: "nested-sudo-password",
        }),
        socks5ProxyChain: [{ host: "proxy.example", token: "nested-token" }],
        portKnockSequence: [{ port: 7000, protocol: "tcp" }],
      },
      "view",
    );
    expect(result.password).toBeUndefined();
    ...
    expect(result.tunnelConnections).toEqual([
      { sourcePort: 8080, auth: { username: "root" } },
    ]);
    expect(result.terminalConfig).toEqual({ shell: "bash" });
    expect(result.socks5ProxyChain).toEqual([{ host: "proxy.example" }]);
    expect(result.portKnockSequence).toBeUndefined();
